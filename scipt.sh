...
df --local -P | awk {if (NR!=1) print $6} | xargs -I {} find {} -xdev -type f -perm -0002 find  -xdev -type f -perm -0002
...
cat /etc/passwd | egrep -v '^(root|halt|sync|shutdown)' | awk -F: '($7 != "/sbin/nologin" & do
 if [ ! -d "$dir" ]; then
 echo "The home directory ($dir) of user $user does not exist."
 else
